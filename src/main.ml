...
let base_command_parameters : (unit -> 'result) Command.Param.t =
  [%map_open
    ...
    and specification_directories =
      flag
        "templates"
        (optional (Arg_type.comma_separated string))
        ~doc:"path CSV of directories containing templates"
    and file_filters =
      flag
        "extensions"
                ...
                List.map l ~f:(fun pattern ->
                    if String.contains pattern '/' then
                      ...
                      | exception Unix.Unix_error _ ->
                          Format.eprintf
                            "No such file or directory: %s. Comby interprets patterns containing \
                             '/' as file paths. If a pattern does not contain '/' (like '.ml'), it \
                             is considered a pattern where file endings must match the pattern. \
                             Please supply only valid file paths or patterns.@."
                            pattern;
                          exit 1
                      | path -> path
                    else
    ...
    fun () ->
      ...
      | Some ".generic" ->
          Format.eprintf
            "@.WARNING: the GENERIC matcher was used, because a language could not be inferred \
             from the file extension(s). The GENERIC matcher may miss matches. See '-list' to set \
             a matcher for a specific language and to remove this warning.@."
      | Some extension ->
          let (module M) = configuration.matcher in
          if M.name = "Generic" then
            Format.eprintf
              "@.WARNING: the GENERIC matcher was used because I'm unable to guess what language \
               to use for the file extension %s. The GENERIC matcher may miss matches. See '-list' \
               to set a matcher for a specific language and to remove this warning.@."
              extension
          else if debug then
            Format.eprintf
              "@.NOTE: the %s matcher was inferred from extension %s. See '-list' to set a matcher \
...
let () =
  ...
  Command.run default_command ~version:"0.15.0"
