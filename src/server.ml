...
let check_too_long s =
  ...
  if n > max_request_length then
    Error
      (Format.sprintf
         "The source input is a bit big! Make it %d characters shorter, or click 'Run in Terminal' \
          below to install and run comby locally :)"
...
let perform_match request =
  ...
  | Ok ({ source; match_template; rule; language; id } as request) ->
      if debug then
        Format.printf
          "Received %s@."
          (Yojson.Safe.pretty_to_string (In.match_request_to_yojson request));
      let matcher =
...
let perform_rewrite request =
  ...
  >>| function
  | Ok
      ( { source; match_template; rewrite_template; rule; language; substitution_kind; id } as
      request ) ->
      if debug then
        Format.printf
          "Received %s@."
          (Yojson.Safe.pretty_to_string (In.rewrite_request_to_yojson request));
      let matcher =
        match Matchers.select_with_extension language with
        | Some matcher -> matcher
        | None -> (module Matchers.Generic)
      ...
      in
      let default =
        Out.Rewrite.to_string { rewritten_source = ""; in_place_substitutions = []; id }
      in
      let run ?rule () =
        let configuration = Configuration.create ~match_kind:Fuzzy () in
        let matches =
        ...
        |> Option.value_map
             ~default
             ~f:(fun Replacement.{ rewritten_source; in_place_substitutions } ->
               Out.Rewrite.to_string { rewritten_source; in_place_substitutions; id })
      in
      let code, result =
        match Option.map rule ~f:Rule.create with
...
let perform_environment_substitution request =
  ...
  | Ok ({ rewrite_template; environment; id } as request) ->
      if debug then
        Format.printf
          "Received %s@."
          (Yojson.Safe.pretty_to_string (In.substitution_request_to_yojson request));
      let code, result =
        ( 200
        , Out.Substitution.to_string
            { result = fst @@ Rewrite_template.substitute rewrite_template environment; id } )
      in
      if debug then Format.printf "Result (%d) %s@." code result;
      respond ~code:(`Code code) (`String result)
  | Error error ->
...
let allow_cors =
  let filter handler req =
    handler req
    >>| fun response ->
    response |> Response.headers |> add_cors_headers |> Field.fset Response.Fields.headers response
  in
...
let () =
  (Lwt.async_exception_hook :=
     ...
     | Unix.Unix_error (error, func, arg) ->
         Logs.warn (fun m ->
             m "Client connection error %s: %s(%S)" (Unix.error_message error) func arg)
