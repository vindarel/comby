...
let read_with_timeout read_from_channel =
  ...
  let read_from_channel =
    Unix.select ~read:[read_from_fd] ~write:[] ~except:[] ~timeout:(`After (Time.of_int_sec 2)) ()
...
let run ~iterations ~command =
  List.fold (List.init iterations ~f:ident) ~init:Statistics.empty ~f:(fun statistics _ ->
      Statistics.merge (read_stats command) statistics)
...
let bench_diff ~iterations baseline_command new_command =
  ...
  if delta_x < 1.0 then begin
    ...
    if percentage_delta > epsilon_warn then begin
      Format.printf "FAIL: benchmark epsilon exceeded (%.4f > %.4f)@." percentage_delta epsilon_warn;
      1
    end
    else begin
...
let with_zip dir f =
  ...
  | Ok () ->
      ...
      (* XXX for some reason Unix.system does not give enough time, and we can run into
         '"/tmp/bench_.tmp.efb1e5_comby/master.zip: No such file or directory"'. This is a hack.
         Actually test for the file and wait *)
      Unix.sleep 2;
...
let with_master_comby dir f =
  ...
  | Ok () -> (
      ...
      match
        Unix.system
          (Format.sprintf
             "git clone --depth=50 --branch=master https://github.com/comby-tools/comby.git \
              %s/comby-master && make -C %s/comby-master release && cp %s/comby-master/comby %s"
             dir
             dir
             dir
             baseline_comby)
...
let zip_bench () =
  ...
  let go baseline_binary new_binary zip_file =
    let command_args =
      Format.sprintf
        "'%s' '%s' .ml,.mli -matcher .ml -sequential -stats -zip %s > /dev/null"
        match_template
        rewrite_template
        zip_file
    in
