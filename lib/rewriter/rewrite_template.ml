...
let of_match_context
    {
      range = { match_start = { offset = start_index; _ }; match_end = { offset = end_index; _ } }
    ; _
    }
    ~source
  =
...
let get_offsets_for_holes rewrite_template variables =
  ...
  List.fold
    sorted_variables
    ~init:(rewrite_template, [])
    ~f:(fun (rewrite_template, acc) variable ->
      match String.substr_index rewrite_template ~pattern:(":[" ^ variable ^ "]") with
      | Some index ->
          let rewrite_template =
            String.substr_replace_all rewrite_template ~pattern:(":[" ^ variable ^ "]") ~with_:""
          in
