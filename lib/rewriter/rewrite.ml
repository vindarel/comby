...
let substitute_match_contexts (matches : Match.t list) source replacements =
  ...
  let rewrite_template, environment =
    List.fold2_exn
      ...
      ~f:
        (fun (rewrite_template, accumulator_environment)
        ...
        let accumulator_environment =
          Environment.add accumulator_environment hole_id replacement_content
        in
  ...
  let offsets =
    Rewrite_template.get_offsets_for_holes rewrite_template (Environment.vars environment)
  in
  if debug then
    Format.printf
      "Replacements: %d | Offsets 1: %d@."
      (List.length replacements)
      (List.length offsets);
  let offsets = Rewrite_template.get_offsets_after_substitution offsets environment in
  if debug then
    Format.printf
      "Replacements: %d | Offsets 2: %d@."
      (List.length replacements)
      (List.length offsets);
...
(* store range information for this match_context replacement: (a) its offset in the original source
   (b) its replacement context (to calculate the range) (c) an environment of values that are
   updated to reflect their relative offset in the rewrite template *)
let substitute_in_rewrite_template rewrite_template ({ environment; _ } : Match.t) =
  let replacement_content, vars_substituted_for =
    Rewrite_template.substitute rewrite_template environment
  in
  let offsets =
    Rewrite_template.get_offsets_for_holes rewrite_template (Environment.vars environment)
  in
  ...
  let environment =
    List.fold offsets ~init:(Environment.create ()) ~f:(fun acc (var, relative_offset) ->
        if List.mem vars_substituted_for var ~equal:String.equal then
          ...
          let start_location = Location.{ default with offset = relative_offset } in
          let end_location =
            let offset = relative_offset + String.length value in
            Location.{ default with offset }
          in
          let range = Range.{ match_start = start_location; match_end = end_location } in
