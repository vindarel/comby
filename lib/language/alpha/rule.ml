...
let create rule =
  let operator_parser =
    ...
    >> atom_parser
    << spaces
    >>= fun right -> make_equality_expression operator left right << spaces
  in
  ...
  and match_pattern_parser s =
    let case_parser : (atom * expression list, unit) parser =
      ...
      << spaces
      >>= fun antecedent ->
      spaces >> comma_sep expression_parser << spaces |>> fun consequent -> antecedent, consequent
    in
    let match_pattern =
      let pattern keyword =
        string keyword
        ...
        << char '{'
        << spaces
        >>= fun atom -> many1 case_parser << char '}' << spaces >>= fun cases -> return (atom, cases)
      in
  ...
  in
  let rule_parser s =
    (spaces >> string Syntax.rule_prefix >> spaces1 >> comma_sep1 expression_parser << eof) s
  in
