...
let value_parser s = string_literal s

let operator_parser s = (string Syntax.equal <|> string Syntax.not_equal) s
...
let rewrite_template_parser s = (value_parser >>= fun value -> return (RewriteTemplate value)) s
