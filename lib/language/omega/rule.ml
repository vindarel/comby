...
type rewrite_context = { variable: string }
...
let rec apply
  ...
  let rec rule_match ?(rewrite_context : rewrite_context option) env = function
    | True -> true, Some env
    | False -> false, Some env
    | Equal (Variable var, String value)
    | Equal (String value, Variable var) ->
        equal_in_environment var value env
    | Equal (String left, String right) -> String.equal left right, Some env
    | Equal (Variable left, Variable right) ->
        let result =
          Environment.lookup env left
          >>= fun left ->
          Environment.lookup env right >>= fun right -> return (String.equal left right)
        in
        ...
        let result =
          ...
          List.find_map cases ~f:(fun (template, case_expression) ->
              ...
              | String template -> (
                  ...
                  Matcher.all ~configuration ~template ~source
                  |> function
                  | [] -> None
                  | matches ->
              ...
              | Variable _ -> failwith "| :[hole] is invalid. Maybe you meant to put quotes")
        in
    ...
    | RewriteTemplate rewrite_template -> (
        ...
        | Some { variable; _ } ->
            ...
            let env =
              ...
              |> fst
              |> fun replacement' -> Environment.update env variable replacement' |> Option.some
            in
            true, env )
    | Rewrite (Variable variable, (match_template, rewrite_expression)) -> (
        match rewrite_expression with
        | RewriteTemplate rewrite_template ->
            let template =
...
let make_equality_expression operator left right =
  ...
  else if String.equal operator Syntax.not_equal then
...
let create rule =
  let operator_parser =
    ...
    >>= fun operator ->
    spaces *> atom_parser >>= fun right -> make_equality_expression operator left right <* spaces
  in
  ...
  let expression_parser =
    fix (fun expression_parser ->
        let match_pattern_parser =
          let case_parser =
            spaces *> string Syntax.pipe_operator *> spaces *> atom_parser
            <* spaces
            <* string Syntax.arrow
            <* spaces
            >>= fun antecedent ->
            spaces *> sep_by (char ',') expression_parser
            <* spaces
            |>> fun consequent -> antecedent, consequent
          in
          let pattern keyword =
            string keyword *> spaces *> atom_parser
            <* spaces
            <* char '{'
            <* spaces
            >>= fun atom ->
            many1 case_parser <* char '}' <* spaces >>= fun cases -> return (atom, cases)
          in
          pattern Syntax.start_match_pattern |>> fun (atom, cases) -> Match (atom, cases)
        in
        let rewrite_pattern_parser =
          string Syntax.start_rewrite_pattern *> spaces *> atom_parser
          <* spaces
          <* char '{'
          <* spaces
          >>= fun atom ->
          ...
          <* spaces
          >>= fun match_template ->
          spaces *> rewrite_template_parser
          <* spaces
          <* char '}'
          <* spaces
          |>> fun rewrite_template -> Rewrite (atom, (match_template, rewrite_template))
        in
        choice [match_pattern_parser; rewrite_pattern_parser; operator_parser; true'; false'])
  in
  let rule_parser =
    spaces
