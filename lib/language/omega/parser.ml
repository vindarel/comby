...
let variable_parser =
  string Syntax.variable_left_delimiter *> (many (alphanum <|> char '_') |>> String.of_char_list)
...
let char_token_s =
  char '\\' *> escaped_char_s
  >>= (fun c -> return (Format.sprintf {|\%c|} c))
  <|> (any_char |>> String.of_char)


let value_parser = string {|"|} *> many_till char_token_s (string {|"|}) |>> String.concat
...
let rewrite_template_parser = value_parser >>= fun value -> return (RewriteTemplate value)
