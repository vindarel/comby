...
let fast_line_col_compute = Sys.getenv "FAST_LINE_COL_COMPUTE_COMBY" |> Option.is_some
...
let apply_rule ?(substitute_in_place = true) matcher rule matches =
  ...
  List.filter_map matches ~f:(fun ({ environment; _ } as matched) ->
      ...
      (if sat then env else None) >>| fun environment -> { matched with environment })
...
let line_map source =
  ...
  let f sum char =
    ...
    | '\n' ->
        ...
        sum + 1
    | _ ->
        if sum = total_len - 1 then begin
          (* If it's the last char and wasn't a newline, record this offset. *)
          a.(!line_index) <- sum + 1;
          (* FIXME: no test fails if + 1 is removed. Be ware. *)
          sum + 1
        end
        else
          sum + 1
...
let compute_line_col_fast a offset =
  ...
  (* make offset 0-based because line_map is 0-based *)
  ...
  line + 1, col + 1 (* reset 0-based offset to 1 *) + 1
...
let output_result output_printer source_path source_content result =
  match result with
  | Nothing -> ()
  | Matches (matches, _) -> output_printer (Printer.Matches { source_path; matches })
  ...
  let result, count =
    List.fold specifications ~init:(Nothing, 0) ~f:(fun (result, count) specification ->
        ...
        | Replacement (r, content, number_of_matches) ->
            Replacement (r, content, number_of_matches), count + number_of_matches)
  in
...
let process_paths ~sequential ~f paths scheduler =
  let process_bucket ~init paths =
    List.fold ~init paths ~f:(fun count path -> count + f ~input:(`Path path) ~path:(Some path))
  in
  if sequential then
...
let run
  ...
  let count =
    if Option.is_none interactive_review then
      match sources with
      | `String source ->
          with_scheduler scheduler ~f:(fun _ -> per_unit ~input:(`String source) ~path:None)
      ...
      let rewrites, count =
        with_scheduler scheduler ~f:(fun _scheduler ->
            ...
            | `Paths paths ->
                ...
                in
                let count, inputs =
                  process_paths_for_interactive ~sequential ~f:with_rewrites paths scheduler
                in
                count, inputs
