...
module Make (Syntax : Syntax.S) (Info : Info.S) = struct
  ...
  let escapable_string_literal_parser (f : 'a literal_parser_callback) =
    ...
    | Some { delimiters; escape_character } ->
        List.map delimiters ~f:(fun delimiter ->
            let module M = Parsers.String_literals.Alpha.Escapable.Make (struct
  ...
  let raw_string_literal_parser (f : 'a literal_parser_callback) =
    List.map Syntax.raw_string_literals ~f:(fun (left_delimiter, right_delimiter) ->
        let module M = Parsers.String_literals.Alpha.Raw.Make (struct
    ...
    | syntax ->
        List.map syntax ~f:(function
            | Multiline (left, right) ->
                let module M = Parsers.Comments.Alpha.Multiline.Make (struct
                  let left = left
            ...
            | Nested_multiline (left, right) ->
                let module M = Parsers.Comments.Alpha.Nested_multiline.Make (struct
                  let left = left
                ...
                M.comment
            | Until_newline start ->
                let module M = Parsers.Comments.Alpha.Until_newline.Make (struct
                  let start = start
                end)
                in
                M.comment)
  ...
  (** Generates a word that is NOT OK with having alphanum chars before or after it. This disables
      substring parsing. *)
  ...
  let generate_string_token_parser str : ('c, _) parser =
    many comment_parser >> string str >> many comment_parser >>= fun result -> f result


  let is_optional () = opt false (char '?' |>> fun _ -> true)

  let identifier () = many (alphanum <|> char '_') |>> String.of_char_list

  let hole_body () =
    is_optional ()
    >>= fun optional -> identifier () >>= fun identifier -> return (optional, identifier)


  let everything_hole_parser () = string ":[" >> hole_body () << string "]"

  let expression_hole_parser () = string ":[" >> hole_body () << string ":e" << string "]"

  let non_space_hole_parser () = string ":[" >> hole_body () << string ".]"

  let line_hole_parser () = string ":[" >> hole_body () << string "\\n]"

  let blank_hole_parser () =
    string ":["
    >> is_optional ()
    >>= fun optional ->
    many1 blank >> identifier () >>= fun identifier -> string "]" >> return (optional, identifier)


  let alphanum_hole_parser () = string ":[[" >> hole_body () << string "]]"
  ...
  let reserved_holes () =
    ...
    [non_space; line; blank; alphanum; expression; everything]
  ...
  let reserved_delimiters () =
    ...
    let handle_alphanum_delimiters_reserved_trigger from until =
      let from_parser s =
        ...
        | Some prev when is_alphanum (Char.to_string prev) -> fail "unsat"
        | _ -> string from >>= fun from -> look_ahead required_from_suffix >>= fun _ -> return from
        )
          s
      in
      let until_parser s =
    ...
    let reserved_escapable_strings =
      match Syntax.escapable_string_literals with
      | Some { delimiters; _ } ->
          List.concat_map delimiters ~f:(fun delimiter -> [delimiter]) |> List.map ~f:string
      | None -> []
    in
    let reserved_raw_strings =
      ...
      |> List.map ~f:string
    in
    reserved_holes () @ reserved_delimiters @ reserved_escapable_strings @ reserved_raw_strings
    |> List.map ~f:skip
    (* Attempt the reserved: otherwise, if a parser partially succeeds, it won't detect that single
       or greedy is reserved. *)
    |> List.map ~f:attempt
    |> choice


  let reserved _s = reserved_delimiters () <|> skip (space |>> Char.to_string)
  ...
  let record_matches identifier p : ('c, Match.t) parser =
    ...
    update_user_state (fun ({ Match.environment; _ } as result) ->
        if debug then begin
  ...
  let generate_delimited_hole_parser
    ...
    let reserved =
      ...
      |> choice
    in
    (* A parser that understands the hole matching cut off points happen at delimiters. *)
    let rec nested_grammar s =
      ...
      (* Only consume if not reserved. If it is reserved, we want to trigger the 'many' in (many
         nested_grammar) to continue. *)
  ...
  let generate_everything_hole_parser
    ...
    let handle_alphanum_delimiters from until p =
      (* mandatory_prefix: needs to be not alphanum AND not non-alphanum delim. If it is a paren, we
         need to fail and get out of this alphanum block parser (how did we end up in here? because
         we said that we'd accept anything as prefix to 'def', including '(', and so '(' is not
         handled as a delim.) *)
      let mandatory_prefix = alphanum_delimiter_must_satisfy in
      (* mandatory_suffix: be more strict with suffix of opening delimiter: don't use 'any
         non-alphanum', but instead use whitespace. This since 'def;' is undesirable, and 'def.foo'
         may be intentional. But 'end.' or 'end;' probably still refer to a closing delim. *)
      let mandatory_suffix = choice reserved_alphanum_delimiter_must_satisfy <|> whitespace in
      let satisfy_opening_delimiter prev =
        ( match prev with
        | Some prev when is_alphanum (Char.to_string prev) -> fail "unsat"
        (* Try parse whitespace, and we want to capture its length, in case this is a space between,
           like 'def def end end'. But in the case where there's no space, it means we have just
           entered the beginning of the hole which may start with the 'd' of 'def', but since we
           already know that the previous char is not alphanum in this branch (so it is a delimiter
           or whitespace) it is OK to continue: in this case, return "". *)
        ...
        >>= fun _ ->
        (* Use look_ahead to ensure that there is, e.g., whitespace after this possible delimiter,
           but without consuming input. Whitespace needs to not be consumed so that we can detect
      ...
      (fun s ->
        ...
        >>= fun close_delimiter -> return ((prefix ^ open_delimiter) ^ in_between ^ close_delimiter)
        )
          s)
    ...
    let handle_alphanum_delimiters_reserved_trigger from until =
      (* If it's alphanum, only consider it reserved if there is, say, whitespace after and so
         handle alternatively. Otherwise, return empty to indicate 'this sequence of characters is
         not reserved'. *)
      let reserved =
        Syntax.user_defined_delimiters
      ...
      List.map [from; until] ~f:(fun delimiter s ->
          ...
          | _ -> string delimiter >>= fun _ -> look_ahead required_delimiter_terminal )
            s)
    in
    (* The cases for which we need to stop parsing just characters and consider delimiters. *)
    let reserved =
  ...
  let coalesce_whitespace prefix_parser suffix_parser =
    let is_whitespace p =
      match
        ...
        , (* suffix parser could be a hole. It needs to fail on parsing something like "X" to be a
             whitespace parser *)
                ...
                | true -> (
                    ...
                    | suffix :: rest as acc ->
                        let optional_succeeds_parser =
                          record_matches identifier hole_semantics >> sequence_chain acc
                        in
                        let optional_fails_parser =
                          record_matches identifier (return [])
                          >>= fun _ ->
                          if coalesce_whitespace (prefix_parser p_list i) suffix then
                            ...
                            sequence_chain acc
                        in
                        [attempt optional_succeeds_parser <|> optional_fails_parser] ) )
            | Non_space ->
                let allowed =
                  [skip space; reserved_delimiters ()] |> choice |> is_not |>> Char.to_string
                in
                let rest =
                  match acc with
                  | [] -> eof >>= fun () -> f [""]
            ...
            | Everything -> (
                ...
                | false ->
                    let rest =
                      ...
                      | _ -> sequence_chain acc
                    in
                    (* Continue until rest, but don't consume rest. acc will propagate the rest that
                ...
                | true -> (
                    ...
                    | [] ->
                        ...
                        let hole_semantics = many (not_followed_by rest "" >> matcher) in
                        (* Try match ordinary hole semantics, but if the parser fails, just let it
                           pass which leads to assigning "" to identifier *)
                        let hole_semantics = opt [] (attempt hole_semantics) in
                        record_matches identifier hole_semantics :: acc
                    | suffix :: rest as acc ->
                        let after = sequence_chain acc in
                        let hole_semantics = many (not_followed_by after "" >> matcher) in
                        (* The logic goes: Try to match ordinary hole semantics, and propagate acc
                           if it succeeds. If ordinary semantics fail, let it pass and coalesce
                           whitespace with prefix/suffix if needed and only propagate 'rest', since
                           we remove the suffix by coalescing. *)
                        let optional_succeeds_parser =
                          (* This parser can succeed but does not consume after. After must still be
                             consumed (acc must be propagated). *)
                        ...
                        let optional_fails_parser =
                          ...
                          (* The optional parser that kicks in if optional_succeeds_parser fails. It
                             does not consume anything. *)
                          record_matches identifier (return [])
                          >>= fun _ ->
                          (* Record matches succeeded for optional hole, empty match. No going back
                             now. Consume suffix if prefix and suffix are whitespace and propagate
                             rest. Otherwise, propagate acc. *)
                          if coalesce_whitespace (prefix_parser p_list i) suffix then
                            sequence_chain rest
                          else
                            sequence_chain acc
  ...
  let hole_parser sort dimension =
    ...
    hole_parser
    |>> fun (optional, identifier) -> skip_signal { sort; identifier; dimension; optional }


  let generate_hole_for_literal sort ~contents ~left_delimiter ~right_delimiter s =
    let holes = Hole.sorts () |> List.map ~f:(fun kind -> attempt (hole_parser kind sort)) in
    let reserved_holes = reserved_holes () |> List.map ~f:skip |> List.map ~f:attempt |> choice in

    let p =
      many
        ( choice holes
        <|> (many1 (is_not reserved_holes) |>> String.of_char_list |>> generate_string_token_parser)
        )
    in
  ...
  let rec generate_parsers s = many (common s)

  and common _s =
    let holes = Hole.sorts () |> List.map ~f:(fun kind -> attempt (hole_parser kind Code)) in
    choice
      [
        choice holes
        (* String literals are handled specially because match semantics change inside string
           delimiters. *)
      ...
      ; nested_delimiters_parser generate_outer_delimiter_parsers
        (* Whitespace is handled specially because we may change whether they are significant for
           matching. *)
      ...
      begin
        ...
        let p =
          if is_alphanum left_delimiter then
            (* This logic is needed for cases where we say 'def :[1] end' in the template, and don't
               match partially on, say, 'adef body endq' in the underlying generated parser. *)
            let prev = prev_char s in
            match prev with
            | Some prev when is_alphanum (Char.to_string prev) -> fail "unsat"
      ...
      let inner_p =
        ...
        let matcher : ('a, Match.t) parser =
          let with_positions (matcher : ('a, Match.t) parser) : ('a, Match.t) parser =
            ...
            update_user_state (fun result -> { result with range }) >> return Unit
          in
  ...
  let first' shift p source : Match.t Or_error.t =
    ...
    | Success (_, result) ->
        if source = "" then
          (* If source is empty and p succeeds, it's the trivial case. We set the result manually. *)
          Ok
            {
              result with
              range =
