...
let record_match_context pos_before pos_after =
  ...
  let match_context =
    ...
    Match.
      { range = { match_start; match_end }; environment = !current_environment_ref; matched = text }
  in
  (* substitute now *)
  if debug then
  ...
  (* Don't just append, but replace the match context including constant strings. I.e., somewhere
     where we are appending the parth that matched, it shouldn't, and instead just ignore. *)
...
module Make (Syntax : Syntax.S) (Info : Info.S) = struct
  ...
  let f acc (production : production) =
    ...
    | String s ->
        (* unmatched, append when we rewrite *)
        if rewrite then Buffer.add_string actual s;
        acc
    | Template_string _ ->
        acc (* matched. if a constant string in the template is matched, don't append it *)
    | Unit ->
        if debug then Format.printf "Unit@.";
        acc
    | Hole _ ->
        if debug then Format.printf "Hole@.";
        acc
    | Match _ ->
  ...
  let escapable_string_literal_parser (f : 'a literal_parser_callback) =
    ...
    | Some { delimiters; escape_character } ->
        List.map delimiters ~f:(fun delimiter ->
            escapable delimiter escape_character
            >>= fun contents ->
  ...
  module Deprecate = struct
    ...
    let reserved =
      reserved_delimiters @ [" "; "\n"; "\t"; "\r"]
      |> List.sort ~compare:(fun v2 v1 -> String.length v1 - String.length v2)
  end
  ...
  let reserved_parsers =
    ...
    let user_defined_escapable_strings =
      match Syntax.escapable_string_literals with
      | Some { delimiters; _ } -> List.concat_map delimiters ~f:(fun delimiter -> [delimiter])
      | None -> []
    in
  ...
  let generate_everything_hole_parser
    ...
    let between_nested_delims p =
      ...
      in
      parsers |> List.map ~f:fst |> List.map ~f:(between_nested_delims p) |> choice
    in
    let reserved =
      let parsers =
        match left_delimiter, right_delimiter with
        ...
        let result =
          ...
          | Ok (Hole { sort; identifier; dimension; _ }, user_state) -> (
              ...
              | Alphanum ->
                  ...
                  acc
                  >>= fun _ ->
                  let m = { offset; identifier; text = String.concat value } in
              ...
              | Non_space ->
                  ...
                  let rest =
                    (* if this is the base case (the first time we go around the loop backwards,
                       when the first parser is a hole), then it means there's a hole at the end
                       without anything following it in the template. So it should always match to
                       end_of_input, not empty string. If it matches to empty string it chops up the
                       matches so that f,o,o are three matches of foo. *)
                    if !i = 0 then (
                  ...
                  let offset =
                    ...
                    | offset ->
                        ...
                        offset
                  in
                  let m = { offset; identifier; text = String.of_char_list value } in
                  r user_state (Match m)
              | Line ->
                  pos
                  >>= fun offset ->
              ...
              | Everything ->
                  ...
                  let hole_matcher =
                    many_till
                      ...
                      if debug then Format.printf "Pos is %d@." pos;
                      rest )
                    (* it may be that the many till for the first parser succeeds on 'empty string',
                       specifically in the :[1]:[2] case for :[1]. We won't capture the pos of :[1]
                       in the first parser since it doesn't fire, so we have to set the pos right
                       before the until parser below, if that happens. *)
                    >>| String.concat
                  in
                  hole_matcher
                  >>= fun text ->
                  let offset =
                    ...
                    | offset ->
                        ...
                        offset
                  in
                  let m = { offset; identifier; text } in
                  r user_state (Match m) )
          | Ok (_, _user_state) ->
              failwith "unreachable: _signal_hole parsed but not handled by Hole variant"
        in
        i := !i + 1;
        result)


  (* XXX change ignore to unit once everything works. right now it's the string that was parsed by
     spaces1 *)
  let generate_spaces_parser _ignored =
    ...
    >>= fun s1 ->
    many comment_parser
    >>= fun result -> spaces >>= fun s2 -> r acc (Template_string (s1 ^ String.concat result ^ s2))
  ...
  let generate_string_token_parser str =
    ...
    >>= fun result ->
    many comment_parser
    >>= fun s2 -> r acc (Template_string (String.concat s1 ^ result ^ String.concat s2))


  let single_hole_parser () = string ":[[" *> identifier_parser () <* string "]]"

  let everything_hole_parser () = string ":[" *> identifier_parser () <* string "]"

  let non_space_hole_parser () = string ":[" *> identifier_parser () <* string ".]"
  ...
  let generate_hole_for_literal sort ~contents ~left_delimiter ~right_delimiter () =
    let literal_holes =
      ...
      |> choice
    in
    let _reserved_holes = reserved_holes () |> List.map ~f:skip_unit |> choice in
    let parser =
      many
      @@ choice
           [
    ...
    | Error _ ->
        failwith
          "If this failure happens it is a bug: Converting a quoted string in the template to a \
           parser list should not fail here"


  let general_parser_generator : (production * 'a) t t =
    let spaces : (production * 'a) t t = spaces1 |>> generate_spaces_parser in
    let other =
      many1 (Parser.Deprecate.any_char_except ~reserved:Deprecate.reserved)
      |>> String.of_char_list
      |>> generate_string_token_parser
    in
    let code_holes = Hole.sorts () |> List.map ~f:(fun kind -> hole_parser kind Code) |> choice in
    fix (fun (generator : (production * 'a) t list t) ->
        ...
        @@ choice
             ...
             ; escapable_string_literal_parser
                 (generate_hole_for_literal Escapable_string_literal ())
             ; spaces
             ; nested
             ; other
             ]
    ...
    match p_list with
    | [] ->
        (* The template is the empty string and source is nonempty. We need to detect it here or we
           will always match successfully on empty string and never advance input below. *)
        r acc Unit
    | p_list -> (
        p_list
        |> sequence_chain
        ...
        | Exact ->
            ...
            matcher
            >>= fun _access_last_production_here ->
            pos
            >>= fun end_pos ->
            end_of_input
            >>= fun _ ->
        ...
        | Fuzzy ->
            ...
            let matches =
              many
              @@ many_till
                   (prefix >>= fun s -> r acc (String s))
                   ( at_end_of_input
                   >>= fun at_end ->
                   if debug then Format.printf "We are at the end? %b.@." at_end;
                   if at_end then
                     fail "end"
                   else (* We may have found a match *)
                     pos
                     >>= fun start_pos ->
                     let matched =
                       matcher
                     ...
                     let no_match =
                       ...
                       current_environment_ref := Match.Environment.create ();
                       (* cannot return: we must try some other parser or else we'll infini loop! We
                          can't advance because we haven't successfuly parsed the character at the
                          current position. So: fail and try another parser in the choice. *)
  ...
  let to_template template =
    ...
    | Buffered.Done ({ len; buf; _ }, p) ->
        ...
        if len <> 0 then
          failwith @@ Format.sprintf "Input left over in template where not expected: %d: %c" len c;
        Ok p
  ...
  let first_is_broken ?configuration:_ ?shift:_ template source : Match.t Or_error.t =
    ...
    | Error e -> Error e
    | Ok p -> (
        match run_the_parser_for_first p source with
        | Ok _ -> (
            (* May have matches, ok to access *)
  ...
  let set_rewrite_template rewrite_template' = rewrite_template := rewrite_template'

  (** Hardcoded case when template and source are empty string. The parser logic is too tricky for
      this right now. *)
  let trivial =
    let open Match in
    let open Location in
    let open Range in
    let location = { offset = 0; line = 1; column = 1 } in
    let range = { match_start = location; match_end = location } in
    Match.create ~range ()
  ...
  let all ?configuration ~template ~source : Match.t list =
    ...
    if template = "" && source = "" then
      ...
      match first_is_broken template source with
      | Ok _
      | Error _ ->
