...
let dont_use_any_char_except_parser p =
  ...
  let c =
    choice
      [
        ...
        pos
        >>= fun po ->
        ( if debug then Format.printf "1. stop @@ %s @@ %d@." reserved po;
      ...
      ; ( return ()
        ...
        Format.printf "X@.";
        if get_stop () then (
          if debug then Format.printf "2. stop@.";
          fail "stop" )
        else
          any_char )
      ]
  ...
  c
  >>= fun c' ->
  if debug then Format.printf "Parsed: %c@." c';
  if debug then Format.printf "Exit@.";
  return c'


let dont_use_is_not p = dont_use_any_char_except_parser p
...
(* let any_allowed_except_parser allowed p = let rewind = ref false in let set_rewind v = rewind :=
   v in let get_rewind () = !rewind in choice [ (p >>= fun _ -> (return (set_rewind true)) >>= fun _
   -> fail "bad") ; (return () >>= fun _ -> if get_rewind () then fail "rewind" else allowed) (*
...
let skip_unit p = p |>> ignore

module Deprecate = struct
  (* XXX can shortcircuit *)
  (* what if you hit a reserved seqence "{" and then attempt ":[[" and then say "end of input" and
     then move ahead any_char. not good. going from longest to shortest works though *)
...
(** must have at least one, otherwise spins on the empty string *)
let spaces1 =
  satisfy is_whitespace
  >>= fun c ->
  (* XXX use skip_while once everything works. we don't need the string *)
  take_while is_whitespace >>= fun s -> return (Format.sprintf "%c%s" c s)
