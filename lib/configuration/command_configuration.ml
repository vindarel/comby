...
let parse_template_directories ?(warn = false) paths =
  let parse_directory path =
    ...
    | Some match_template ->
        ...
        Specification.create ~match_template ?rule ?rewrite_template () |> Option.some
  in
...
module Printer = struct
  ...
  module Rewrite : sig
    ...
    let print { output_format; substitution_kind } path replacements rewritten_source source_content
      ...
      | Match_only ->
          print_if_some
          @@ Diff_configuration.get_diff Match_only path rewritten_source source_content
      | Json_lines kind ->
          let open Option in
          let to_json diff =
...
let emit_errors { input_options; run_options = _; output_options } =
  let error_on =
    ...
    ; ( Option.is_some input_options.specification_directories
        && List.exists (Option.value_exn input_options.specification_directories) ~f:(fun dir ->
  ...
  let warn_on =
    [
      ( (let match_templates =
           ...
           | None, Some { match_template; _ } -> [match_template]
    ...
    ; ( is_some input_options.specification_directories && is_some input_options.anonymous_arguments
  ...
  if Option.is_some custom_matcher then
    ...
    let matcher =
      ...
      | None when matcher_override <> ".generic" ->
          Format.eprintf
            "The matcher %S is not supported. See -list for supported matchers@."
            matcher_override;
          exit 1
...
let create
    ( {
      ...
      ; output_options =
          { overwrite_file_in_place; color; count; interactive_review; _ } as output_options
      } as configuration )
    : t Or_error.t
  =
  ...
  let output_printer printable =
    ...
    | Matches { source_path; matches } ->
        ...
        |> fun match_output -> Printer.Match.print match_output source_path matches
    | Replacements { source_path; replacements; result; source_content } ->
        ...
        if match_only && color then
          Printer.Rewrite.print
            { replacement_output with output_format = Match_only }
            source_path
            replacements
            result
            source_content
  ...
  in
  let ((module M) as matcher), extension =
    select_matcher custom_matcher override_matcher file_filters
  in
  return
    {
      matcher
